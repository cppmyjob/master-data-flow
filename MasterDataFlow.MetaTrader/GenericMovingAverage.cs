...
namespace MasterDataFlow.MetaTrader
{
    internal class GenericMovingAverage : CumulativeAnalyzableBase<decimal?, decimal?>
    {
        ...
        public GenericMovingAverage(int initialValueIndex, Func<int, decimal?> initialValueFunction,
            Func<int, decimal?> indexValueFunction, Func<int, decimal> smoothingFactorFunction, int inputCount)
            : base(new decimal?[inputCount])
        {
            _initialValueIndex = initialValueIndex;
            _inputFunction = i => i < initialValueIndex ? null :
                                 initialValueIndex == i ? initialValueFunction(i) : indexValueFunction(i);
            _smoothingFactorFunction = smoothingFactorFunction;
        }

        public GenericMovingAverage(Func<int, decimal?> initialValueFunction, Func<int, decimal?> indexValueFunction,
            Func<int, decimal> smoothingFactorFunction, int inputCount)
            : this(0, initialValueFunction, indexValueFunction, smoothingFactorFunction, inputCount)
        {
        }

        public GenericMovingAverage(Func<int, decimal?> indexValueFunction, Func<int, decimal> smoothingFactorFunction,
            int inputCount)
        ...
        protected override decimal? ComputeInitialValue(IReadOnlyList<decimal?> mappedInputs, int index) =>
            _inputFunction(index);

        protected override decimal? ComputeCumulativeValue(IReadOnlyList<decimal?> mappedInputs, int index,
            decimal? prevOutputToMap)
